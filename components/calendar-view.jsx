...
import { Box, Text, Loader, Button, Modal, Label, Input, Icon } from '@adminjs/design-system';
...
const CalendarView = (props) => {
    ...
    const confirmReschedule = async () => {
        try {
            ...
            const response = await fetch(`/admin/api/slotiva/appointments/${selectedEvent.id || selectedEvent.bookingId}`, {
                method: 'PUT',
                headers: {
    ...
    const confirmCancel = async () => {
        ...
        try {
            const response = await fetch(`/admin/api/slotiva/appointments/${selectedEvent.id || selectedEvent.bookingId}/cancel`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <Box style={{ padding: '20px', backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
            ...
            >
                ...
                {/* Legend */}
                <Box flex style={{ gap: '16px', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #e2e8f0' }}>
                    <Box flex alignItems="center" style={{ gap: '6px' }}>
                        <Box style={{ width: '12px', height: '12px', borderRadius: '3px', backgroundColor: '#10B981' }} />
                        <Text fontSize="sm" color="grey60">Confirmed</Text>
                    </Box>
                    <Box flex alignItems="center" style={{ gap: '6px' }}>
                        <Box style={{ width: '12px', height: '12px', borderRadius: '3px', backgroundColor: '#EF4444' }} />
                        <Text fontSize="sm" color="grey60">Cancelled</Text>
                    </Box>
                    <Box flex alignItems="center" style={{ gap: '6px' }}>
                        <Box style={{ width: '12px', height: '12px', borderRadius: '3px', backgroundColor: '#3B82F6' }} />
                        <Text fontSize="sm" color="grey60">Completed</Text>
                    </Box>
                    <Box flex alignItems="center" style={{ gap: '6px' }}>
                        <Box style={{ width: '12px', height: '12px', borderRadius: '3px', backgroundColor: '#F59E0B' }} />
                        <Text fontSize="sm" color="grey60">Pending</Text>
                    </Box>
                </Box>
            </Box>
                    ...
                    eventContent={(arg) => {
                        return (
                            <Box style={{ padding: '2px 4px', overflow: 'hidden' }}>
                                ...
                                >
                                    {arg.timeText && <span style={{ marginRight: '4px' }}>{arg.timeText}</span>}
                                    {arg.event.title}
                                </Text>
                            </Box>
                        );
                ...
                >
                    <Box p="lg">
                        <Box style={{ display: 'grid', gap: '16px' }}>
                            {/* Booking ID */}
                            <Box>
                                <Label style={{ marginBottom: '4px', fontSize: '12px', color: '#718096' }}>
                                    Booking ID
                                </Label>
                            ...
                            <Box>
                                ...
                                <Text fontWeight="600">{selectedEvent.clientName || 'N/A'}</Text>
                            </Box>
                            ...
                            {selectedEvent.notes && (
                                <Box>
                                    ...
                                    <Text style={{ whiteSpace: 'pre-wrap' }}>{selectedEvent.notes}</Text>
                                </Box>
                            )}
                        </Box>

                        {/* Actions */}
                        <Box flex justifyContent="flex-end" style={{ gap: '8px', marginTop: '24px' }}>
                            <Button
                    ...
                    <Box p="lg">
                        ...
                        <Box style={{ display: 'grid', gap: '16px' }}>
                            ...
                            <Box>
                                ...
                                <Box flex alignItems="center" style={{ gap: '12px' }}>
                                    <Input
                                        ...
                                        width="100%"
                                    />
                                    <Text style={{ whiteSpace: 'nowrap', fontSize: '13px', color: '#718096' }}>
                            ...
                            {availabilityStatus && (
                                <Box
                                    p="default"
                                    bg={availabilityStatus === 'available' ? 'success20' : 'error20'}
                                    style={{
                                        borderRadius: '4px',
                                ...
                                >
                                    <Box
                                        style={{
                                            ...
                                            height: '8px',
                                            borderRadius: '50%',
                                            backgroundColor: availabilityStatus === 'available' ? '#10B981' : '#EF4444'
                                        }}
                                    />
                                    <Text color={availabilityStatus === 'available' ? 'success100' : 'error100'} fontWeight="500">
                        ...
                        <Box flex justifyContent="flex-end" style={{ gap: '8px', marginTop: '24px' }}>
                            ...
                            <Button
                                ...
                                variant="primary"
                                disabled={!availabilityStatus || availabilityStatus === 'conflict'}
                            >
                                Confirm Reschedule
                            </Button>
                        </Box>
                    ...
                    <Box p="lg">
                        <Box mb="xl" p="default" bg="grey20" style={{ borderRadius: '4px' }}>
                            <Box style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '8px 24px' }}>
                                <Text color="grey60" fontSize="sm">Client:</Text>
                                <Text fontWeight="600" fontSize="sm">{selectedEvent.clientName}</Text>

                                <Text color="grey60" fontSize="sm">Time:</Text>
                                ...
                                <Text color="grey60" fontSize="sm">Service:</Text>
                                <Text fontWeight="600" fontSize="sm">{selectedEvent.serviceType || 'Consultation'}</Text>
                            </Box>
                        </Box>

                        <Box mb="xl">
                            <Text mb="md" fontWeight="600">This action will:</Text>
